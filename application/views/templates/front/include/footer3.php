	...
	<div class="pop-holder">
		<div class="pop-slida">
			<div class="title-pop">
				...
				<strong class="title-ite">Плинтус <?=$item->plintus_type->name; ?> <?=$item->name; ?></strong>
				<a href="/remembered/remember/<?=$catalog_id; ?>/<?=$item->id ?>" class="remo">Запомнить</a>
			</div>
			<div class="gal-info">
				<div class="gal-price">
					<strong class="title">Цена</strong>
					<strong class="price"><?=$this->kurs->create_price($item,'catalog'); ?> <strong class="rub">руб. за пог./м.</strong></strong>
				</div>
				<ul class="tov-feat-list" id="itmcard">
											<li>Длина <?=$item->plintus_length->name; ?></li>
											<li>Профиль <?=$item->plintus_height->name; ?></li>
											<li><?=$item->otdelka->name; ?></li>
											
											<li>Гарантия <?=$item->guarant->name; ?></li>
				</ul>
				<form action="/basket/insert_basket" class="spi-form" method="POST">
					<div class="row">
						...
						<div class="upaka">
							...
							<strong class="valua"><?=$item->m2; ?> шт. </strong>
						</div>
						<div class="kolva">
					...
					<div class="row">
						<a href="<?=$link_id.'/'.$item->brand->alias.'-'.$item->wood->alias.'-'.$item->id.'-id'; ?>" class="green-button-40">Посмотреть</a>
